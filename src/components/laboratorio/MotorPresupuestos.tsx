...
function eur(n: number, locale: string): string {
  return n.toLocaleString(locale === "en" ? "en-US" : "es-DO", {
    style: "currency",
    currency: "USD",
  ...
  return (
    <div className="grid gap-5 lg:grid-cols-[0.95fr_1.05fr] lg:items-start">
      ...
      <div className="surface-card edge-light flex flex-col p-6 sm:p-7">
        <h2 className="font-mono text-2xs tracking-widest text-azure-400 uppercase">
          {t.entrada.titulo}
        </h2>
        ...
        <textarea
          ...
          className="mt-2 w-full resize-y rounded-lg border border-hairline bg-ink-900/70 px-4 py-3 text-sm leading-relaxed text-ink-50 transition-colors placeholder:text-faint hover:border-hairline-strong focus:border-azure-500/60 focus:outline-none"
        />
            ...
            {t.ejemplos.map((ej) => (
              <li key={ej.id}>
                <button
                  ...
                  className="group/ej w-full rounded-lg border border-hairline bg-ink-900/40 px-4 py-3 text-left transition-colors hover:border-azure-500/40 hover:bg-ink-800/60"
                >
                  <span className="flex items-center justify-between gap-3">
                    <span className="text-sm font-medium text-ink-100">{ej.etiqueta}</span>
                    <ArrowRight
                      className="size-3.5 shrink-0 text-faint transition-transform duration-300 group-hover/ej:translate-x-0.5 group-hover/ej:text-azure-400"
                      aria-hidden
                    />
                  </span>
      ...
      <div className="flex flex-col gap-5">
        <div className="surface-card edge-light p-6 sm:p-7">
          ...
          <div className="mt-6" aria-live="polite">
            {error ? (
              <p className="flex items-start gap-2.5 rounded-lg border border-alerta-600/40 bg-alerta-900/25 px-4 py-3 text-sm text-alerta-200">
                      ...
                      <ul className="mt-3 flex flex-col gap-2">
                        {avisos.map((a, i) => (
                          <li key={i} className="flex items-start gap-2.5 text-sm text-ink-200">
                            <CircleAlert className="mt-0.5 size-3.5 shrink-0 text-alerta-300" aria-hidden />
                            {a.mensaje}
                          </li>
                        ))}
                      </ul>
...
function Presupuesto({
  ...
  const filas: [string, number, boolean?][] = [
    ...
    [t.resultado.iva, presupuesto.impuesto],
        ...
        {filas.map(([etiqueta, valor, destacada]) => (
          ...
          >
            ...
            <dd
              className={cn(
                "tabular-nums",
                destacada ? "font-display text-2xl text-azure-400" : "text-sm text-ink-100",
              )}
            >
              {eur(valor, locale)}
            </dd>
  ...
  return (
    <div>
      <h3 className="flex items-center gap-2.5 text-xl text-ink-50">
        <span className="flex size-7 shrink-0 items-center justify-center rounded-full bg-alerta-500/15 text-alerta-300">
      ...
      <p className="mt-5 rounded-lg border border-alerta-600/30 bg-alerta-900/20 px-4 py-3 text-sm text-alerta-100">
        {escalado.motivo}
      </p>
          ...
          <ul className="mt-3 flex flex-col gap-2.5">
            {escalado.preguntas.map((p) => (
              <li key={p} className="flex items-start gap-2.5 text-sm text-ink-200">
                <span aria-hidden className="mt-1.5 size-1 shrink-0 rounded-full bg-alerta-400" />
                {p}
              </li>
            ))}
          </ul>
  ...
  return (
    <div className="surface-card edge-light p-6 sm:p-7">
      ...
      <button
        ...
        className="mt-4 inline-flex items-center gap-2 font-mono text-2xs tracking-widest text-faint uppercase transition-colors hover:text-azure-400"
      >
